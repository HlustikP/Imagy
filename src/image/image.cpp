...
int Image::LoadImgData(std::string& filename, ImgFormat format) {
	switch(format) {
		...
		case WEBP: {
			...
			const auto result = Decode(format, reinterpret_cast<uint8_t*>(file_data), webp_length);
...
int Image::Decode(ImgFormat in_format, uint8_t* data, int in_length) {
	switch (in_format) {
	case BMP:
		break;
	case JPG:
	...
	case WEBP: {
		...
		WebPGetFeatures(data, in_length, &config.input);
